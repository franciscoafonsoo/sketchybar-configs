...
if [ "$SENDER" == "mouse.entered" ]; then
  ...
  sketchybar --set "$NAME" \
    ...
    label.color="$LEFT_SELECTED_COLOR" \
    icon.color="$LEFT_SELECTED_COLOR" \
    background.color="$LEFT_SELECTED_BG"
  exit 0
fi
...
if [ "$SENDER" == "mouse.exited" ]; then
  ...
  sketchybar --set "$NAME" \
    background.drawing=on \
    label.color="$LEFT_UNSELECTED_COLOR" \
    icon.color="$LEFT_UNSELECTED_COLOR" \
    background.color="$LEFT_UNSELECTED_BG"
  exit 0
fi
